...
async function sendNotification(title, options = {}, notificationType = "general") {
     if (Notification.permission !== "granted") {
...
async function toggleNotifications() {
 ...
 async function loadNotificationSettings() {
     ...
     const notificationSound = (await db.getSetting("notificationSound")) || "default";
     const notificationVibration = (await db.getSetting("notificationVibration")) !== "false";
     const quietHoursEnabled = (await db.getSetting("quietHoursEnabled")) === "true";
     ...
     document.getElementById("notificationsEnabledToggle").checked = notificationsEnabled;
     ...
     document.getElementById("notificationVibration").checked = notificationVibration;
 ...
 async function saveNotificationSettings() {
     await db.setSetting("notifyNewEntry", document.getElementById("notifyNewEntry").checked ? "true" : "false");
     await db.setSetting("notifyEditEntry", document.getElementById("notifyEditEntry").checked ? "true" : "false");
     await db.setSetting("notifyDeleteEntry", document.getElementById("notifyDeleteEntry").checked ? "true" : "false");
     await db.setSetting("notifyDailySummary", document.getElementById("notifyDailySummary").checked ? "true" : "false");
     await db.setSetting("notificationSound", document.getElementById("notificationSound").value);
     await db.setSetting("notificationVibration", document.getElementById("notificationVibration").checked ? "true" : "false");
     await db.setSetting("quietHoursEnabled", document.getElementById("quietHoursEnabled").checked ? "true" : "false");
     await db.setSetting("quietHoursStart", document.getElementById("quietHoursStart").value);
     await db.setSetting("quietHoursEnd", document.getElementById("quietHoursEnd").value);
 }

 // Handle notifications toggle
 function handleNotificationsToggle() {
     const isEnabled = document.getElementById("notificationsEnabledToggle").checked;
     if (isEnabled) {
         requestNotificationPermission();
     } else {
...
function getThemeColors() {
    ...
    return {
        ...
        textSecondary: styles.getPropertyValue('--text-secondary').trim() || '#701705',
        primaryColor: styles.getPropertyValue('--primary-color').trim() || '#F54927',
        borderColor: styles.getPropertyValue('--border-color').trim() || '#FAA18F',
        successColor: styles.getPropertyValue('--success-color').trim() || '#F87C63'
    };
}
...
async function calculateConsumption() {
    ...
    if (Notification.permission === "granted") {
        ...
        await sendNotification("Fleet Manager", {
...
async function editHistoryItem(id) {
    const record = await db.getFuelEntryById(id);
...
function deleteHistoryItem(id) {
     Swal.fire({
         ...
         if (result.isConfirmed) {
             ...
             const entry = await db.getFuelEntryById(id);
